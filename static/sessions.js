...
async function newSession(flash, options={}){
  ...
  _newSessionInFlight=(async()=>{
    ...
    S.session=data.session;S.messages=data.session.context_messages&&data.session.context_messages.length>data.session.messages.length?data.session.context_messages:data.session.messages||[];
...
async function _ensureMessagesLoaded(sid) {
  ...
  // Prefer context_messages (full pre-compression archive) over messages (compressed API context)
  const msgs = ((data.session.context_messages && data.session.context_messages.length > (data.session.messages||[]).length ? data.session.context_messages : data.session.messages) || []).filter(m => m && m.role);
